...
public class LobbyCanvas : MonoBehaviour {
    ...
    public void ShowCanvas (string header, string desc, string stats, List<string> team1, List<string> team2, bool isOwner, string gameID) {
    ...
    public void UpdateIsOwner (bool isOwner) {
        ownerControls.SetActive (isOwner);
    }
    ...
    public void StartGame () {
        NetworkClient.Send (new StartMinigameMessage () { gameID = gameID });
        MouseController.instance.HideCursor ();
        gameObject.SetActive (false);
    }
    public void LeaveLobby () {
        MouseController.instance.HideCursor ();
        NetworkClient.Send (new LeaveMinigameMessage () { username = PlayerPrefs.GetString ("username"), gameID = gameID });
        gameObject.SetActive (false);
    }
}